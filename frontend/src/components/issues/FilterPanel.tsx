    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" className="gap-2 relative">
          ...
          Filters
          {count > 0 && (
            <Badge className="absolute -top-2 -right-2 w-5 h-5 p-0 flex items-center justify-center text-xs bg-purple-600 text-white">{count}</Badge>
          )}
        </Button>
      </PopoverTrigger>
      ...
      <PopoverContent align="end" className="w-80 p-0" sideOffset={8}>
        ...
        <div className="px-4 py-3 border-t flex justify-between gap-2">
          ...
          <Button size="sm" className="bg-purple-600 hover:bg-purple-700 text-white" onClick={() => setOpen(false)}>
            Apply
          </Button>
        </div>
      </PopoverContent>
